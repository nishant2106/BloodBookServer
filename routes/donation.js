...
donationRouter
  ...
  .post(cors.corsWithOptions, (req, res, next) => {
    ...
    const sqlInsert =
      "INSERT INTO Donation(aadhar_no,component_name,qty,d_date,nurse_id)VALUES(?,?,?,?,?);";
    db.query(
      sqlInsert,
      [adhar_no, component_name, quantity, d_date, nurse_id],
      (err, result) => {
        if (result) {
          res.statusCode = 200;
          res.setHeader("Content-Type", "text/plain");
...
donationRouter
  ...
  .get(cors.corsWithOptions, (req, res, next) => {
    const sqlInsert =
      "select aadhar_no,name from Donor where donationDate < (sysdate() - INTERVAL 3 month);";
    db.query(sqlInsert, (err, result) => {
      res.send(result);
    });
  });
