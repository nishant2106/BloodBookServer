...
donorRouter
  ...
  .post(cors.corsWithOptions, (req, res, next) => {
    ...
    const sqlInsert =
      "INSERT INTO Donor(aadhar_no,name,email,district,blood_grp,mob_no,gender,address,pinCode,isPlasmaDonor)VALUES(?,?,?,?,?,?,?,?,?,?);";
    db.query(
      sqlInsert,
      [
        aadhar,
...
donorRouter
  ...
  .post(cors.corsWithOptions, (req, res, next) => {
    ...
    const sqlUpdate =
      "update Donor set name=?,email=?,district=?,mob_no=?,address=?,pinCode=? where aadhar_no=?;";
    db.query(
      sqlUpdate,
      [name, email, district, mobile, address, pincode, aadhar],
      (err, result) => {
        if (err) {
          console.log(err);
        }
