...
nurseRouter
  ...
  .post(cors.corsWithOptions, (req, res, next) => {
    ...
    const sqlInsert =
      "INSERT INTO Nurse(name,gender,city,mob_no,email,join_date)VALUES(?,?,?,?,?,?);";
    db.query(
      sqlInsert,
      [name, gender, address, mobile, email, joinDate],
      (err, result) => {
        if (result) {
          res.statusCode = 200;
          res.setHeader("Content-Type", "text/plain");
...
nurseRouter
  ...
  .post(cors.corsWithOptions, (req, res, next) => {
    ...
    const sqlInsert =
      "update Nurse set name=?,email=?,mob_no=?,city=? where nurse_id=?;";
